  ...
  if ('locations' in resource) {
    if (Array.isArray(resource.locations)) {
      resource.locations.forEach(location => {
        const contextGroup = makeElement('context-group', { purpose: "location" }, true);
        contextGroup.elements.push(makeElement('context', { "context-type": "sourcefile" }, [makeText(location.sourcefile)]));
        contextGroup.elements.push(makeElement('context', { "context-type": "linenumber" }, [makeText(location.linenumber)]));
        u.elements.push(contextGroup);
      });
    }
  }
